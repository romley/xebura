...
$values = array(XE_GROUP_MID => $mid, XE_GROUP_NAME => $name, XE_GROUP_ADD_DATE => $date);
$gid = $db->insert("XEBURA_EMAIL_GROUP",$values);
echo 'OK';
exit;
	}
	elseif($action=='update'){
		$date = date('Y-m-d G:i:s',(strtotime("now")));
$values = array(XE_GROUP_MID => $mid, XE_GROUP_NAME => $name, XE_GROUP_ADD_DATE => $date);
$cid = $db->update("XEBURA_EMAIL_GROUP",$values,"WHERE XE_GROUP_ID = '".$gid."'");
echo 'OK';
exit;
		}
...
$smarty->display('create_list.tpl');
?>
