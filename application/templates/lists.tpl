...
<title>{if $gid neq ''}{$name}{else}Lists{/if} - Xebura.com</title>
...
<script type="text/javascript" charset="utf-8">
			$(document).ready(function() {
				bTable = $('#boxoffice').dataTable( {
					"bProcessing": true,
					"sAjaxSource": 'email_chart?list=all{/literal}{if $gid neq ''}&i={$gid}{/if}{literal}',
					"aaSorting": [[ 6, "desc" ]],
					"bPaginate": true,
					"bLengthChange": true,
					"bFilter": true,
					...
					$.fn.colorbox({iframe:false,initialWidth:75,initialHeight:75,href:"contact_detail?i="+aData[0]});
					} );
					}
...
<script type="text/javascript" charset="utf-8">
			$(document).ready(function() {
				gTable = $('#group_list').dataTable( {
					"bProcessing": true,
					"sAjaxSource": 'list_chart?list=all',
					"aaSorting": [[ 3, "desc" ]],
					"bPaginate": true,
					"bLengthChange": true,
					"bFilter": true,
					...
					"fnInitComplete": function ( bSettings ) {
					$(gTable.fnGetNodes()).click( function () {
					var iPos = gTable.fnGetPosition( this );
					var gData = bSettings.aoData[ iPos ]._aData;
					/*window.open('artist_sub_scores?artist={/literal}{$artistid}&start={$startdate}&end={$enddate}{literal}&city='+aData[0],'cityreport','width=1020,height=700,resizable=0');*/
					/*$("embed").attr("wmode", "opaque").wrap('<div>');*/
					window.location = "lists?i="+gData[0];
					} );
					}
...
$(function(){
		   ...
		   $.fn.colorbox({iframe:false,initialWidth:75,initialHeight:75,href:"add_contact{/literal}{if $gid neq ''}?i={$gid}{/if}{literal}"});
		   });
		   });

$(function(){
		   $('#createlist').click(function(){
		   $.fn.colorbox({iframe:false,initialWidth:75,initialHeight:75,href:"create_list"});
		   });
		   });
</script>
...
{if $gid eq ''}
<button class="add_contact"  id="createlist" name="Add Contact" value="Add Contact" >Create List</button>
<div id="top-scores">
  <div id="boxreports" style="font-size:12px;font-weight:normal;margin-top:10px;">
    <div id="dynamic">
      <div class="li nocufon" id="boxreports_title" style="width:auto;font-family:Arial, Helvetica, sans-serif;font-size:16px;height:auto;">Mailing Lists</div>
      <table cellpadding="0" cellspacing="0" border="0" class="display" id="group_list" style="font-size:12px;font-weight:normal;">
        <thead>
          <tr style="font-size:12px;" title="Click to sort">
            <th width="0">ID</th>
            <th width="50">List Name</th>
            <th width="200">Created</th>
            <th width="100"># Members</th>
             <th width="100">Last Mailed</th>
...
<button class="add_contact"  id="addcontact" name="Add Contact" value="Add Contact" >Add New Contact</button>
...
<div id="top-scores">
  <div id="boxreports" style="font-size:12px;font-weight:normal;margin-top:10px;">
    <div id="dynamic">
      <div class="li nocufon" id="boxreports_title" style="width:auto;font-family:Arial, Helvetica, sans-serif;font-size:16px;height:auto;">{if $gid neq ''}{$name}{else}Master Contact List{/if}</div>
